...
class Axios {
private:
    std::string api_base = "https://api.axios.com/api";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("api.axios.com"));
...
public:
    ...
    pplx::task<json::value> get_audiences(){
        return make_api_call("/render/audiences","GET");
    }

    pplx::task<json::value> get_topics(bool is_local=true){
        return make_api_call("/render/topics?is_local=" + is_local,"GET");
    }
};
